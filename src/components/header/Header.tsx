...
export function Header() {
  ...
  const { isCollapsed, isMobileOpen, toggleCollapsed, toggleMobile } = useSidebarStore();
  ...
  return (
    <header className="relative bg-gradient-to-r from-white/90 to-cyan-50/90 backdrop-blur-xl border-b border-cyan-200/50 shadow-lg shadow-cyan-900/10">
      <div className="flex items-center justify-between px-6 py-4">
        {/* Left side - Hamburger & Title */}
        <div className="flex items-center gap-4">
          {/* Hamburger Menu */}
          <button
            onClick={() => window.innerWidth >= 768 ? toggleCollapsed() : toggleMobile()}
          ...
          <div>
            ...
            <p className="text-xs text-cyan-600/80 font-medium">Panel de Administración</p>
          </div>
        </div>
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-cyan-200/50 z-50 overflow-hidden">
                <div className="p-2">
                  <div className="px-4 py-3 border-b border-cyan-100">
                    <div className="text-sm font-semibold text-cyan-800">{user.name}</div>
                    <div className="text-xs text-cyan-600">{user.email || 'admin@odinsys.com'}</div>
                  </div>
